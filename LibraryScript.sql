...
ALTER TABLE Borrow
ADD CONSTRAINT fk_Borrow_Material
...
ALTER TABLE Borrow
ADD CONSTRAINT fk_Borrow_Staff
FOREIGN KEY (Staff_ID)
REFERENCES Staff(Staff_ID);

ALTER TABLE Authorship
ADD CONSTRAINT FK_Authorship_Author_ID
...
ALTER TABLE Authorship
ADD CONSTRAINT FK_Authorship_Material_ID
FOREIGN KEY (Material_ID)
REFERENCES Material(Material_ID)
ON UPDATE CASCADE
ON DELETE CASCADE;
...
SELECT Material.Material_ID, Material.Title, Borrow.Borrow_date, Borrow.due_date
FROM Material
...
Select Material.Title, count(Borrow.Material_ID) FROM Borrow join Material on borrow.Material_Id = Material.Material_ID	
Group By Material.Title
Order By count(Borrow.Material_ID) DESC
OFFSET 0 ROWS FETCH FIRST 10 ROWS ONLY


--Q4
--How many books has the author Lucas Piki written?
SELECT COUNT(authorship.Author_ID) AS num_books
FROM Authorship
...
FROM (
  SELECT COUNT(Author_ID) AS Num_Aut, Material_ID
  FROM Authorship
  GROUP BY Material_ID
  HAVING COUNT(Author_ID) > 1
) AS books_with_multiple_authors;


--Q6
--What are the most popular genres in the library?
Select g.Name, count(*) From
Material m join Borrow b ON m.Material_ID = b.Material_ID
...
SELECT COUNT(*) as Num_of_Borrows
FROM borrow
